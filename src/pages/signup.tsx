...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Agar backend o'chiq bo'lsa yoki ulanishda xato bo'lsa, aniqroq xabar beramiz
      if (err.message === 'Network Error') {
        setError("Server bilan aloqa yo'q. Backend ishlayotganini tekshiring.");
      } else {
        setError(err.response?.data?.detail || "Xatolik yuz berdi. Bu ID band bo'lishi mumkin.");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans relative">
      ...
      <div className="bg-white w-full max-w-sm rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] p-8 border border-gray-100">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-black text-gray-800">Ro'yxatdan o'tish</h2>
          <p className="text-gray-500 text-sm mt-2">Yangi talaba profilini yaratish</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">Ism va familiya</label>
            <input 
              type="text" required placeholder="Masalan: Shohjaxon"
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">Talaba ID (Yoki HEMIS)</label>
            <input 
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">Yangi parol o'ylab toping</label>
            <input 
          ...
          <button 
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-4 rounded-2xl shadow-lg shadow-blue-200 transition-all duration-300 active:scale-95 mt-4 disabled:opacity-70"
          >
        ...
        <p className="text-center mt-6 text-sm text-gray-500 font-medium">
          Akkauntingiz bormi? <Link href="/login" className="text-blue-600 font-bold hover:underline">Tizimga kiring</Link>
        </p>
      </div>
    </div>
  );
